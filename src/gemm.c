...
#pragma omp parallel for
    for (i = 0, maxx = -10000, maxA = -10000, minn = 10000, minA = 10000, maxC = -10000, minC = 10000; i < M; ++i)
    {
        ...
        for (j = 0; j < N; ++j)
        {
            maxC = (maxC > C[i * ldc + j]) ? maxC : C[i * ldc + j];
            minC = (minC < C[i * ldc + j]) ? minC : C[i * ldc + j];
        }
    }
                    ...
                    float *C, int ldc)
{
    ...
    g_ptr->ALPHA = ALPHA;
    pthread_t g_thrd;
    if (pthread_create(&g_thrd, 0, gemm_nn_thrd, g_ptr))
        error("Thread creation failed hetero");
...
void gemm_f(int TA, int TB, int TC,
            int M, int N, int K, float ALPHA,
            float *A, int lda,
            float *B, int ldb,
            float BETA,
